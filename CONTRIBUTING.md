...
4.  **Development Server**: Run `npm start`.
5.  **Production Check**: Run `npm run build` before opening a pull request.
...
*   **3D scenes** should use the existing React Three Fiber and Three.js patterns. Preserve reduced-motion fallbacks and mobile particle budgets.
*   **Landing page changes** must preserve the product entry path from `/` to `/auth`.

### Component Structure
*   **Functional Components**: Use React functional components with Hooks.
*   **Props**: Destructure props clearly.
*   **Imports**: Group imports: React -> Library -> Internal Components -> Styles/Assets.
*   **Shared UI**: Extend the existing `ui`, `cinematic`, and `landing` primitives before introducing one-off styling.
