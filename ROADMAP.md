...
-   **Shared UI**: Reusable landing, cinematic, and product primitives now drive every route.
-   **Marketing Surface**: The cinematic landing is integrated at `/` and routes directly to `/auth`.
-   **Deployment**: Vercel is configured for the Vite build and React Router SPA fallback.

### Critical Scaling Gaps
1.  **Data Completion**: Authentication, player stats, and quests use Supabase; secondary dashboard achievements, reminders, and quick stats still use local data.
...
## Phase 3: Server-Side Authority (Anti-Cheat)
...
1.  **Async State**: Add a query/cache layer only when Supabase invalidation and cross-route synchronization require it.
2.  **Performance**: Add route-level code splitting and virtualize long quest or leaderboard lists.
